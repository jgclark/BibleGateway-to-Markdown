...
PASSAGE_RE = '<h1 class="passage-display">.*<div class="footnotes">'.freeze
MATCH_PASSAGE_RE = '<h1 class="passage-display">(.*)<div class="footnotes">'.freeze
...
opt_parser = OptionParser.new do |o|
  ...
  o.separator ''
  opts[:headers] = false
  o.on('-e', '--headers', 'Include editorial headers') do
    opts[:headers] = true
  end
  opts[:footnotes] = false
  o.on('-f', '--footnotes', 'Include footnotes') do
    opts[:footnotes] = true
  end
  ...
  o.on('-n', '--numbering', 'Include verse and chapter numbers') do
    opts[:numbering] = true
  end
...
puts "Found #{input_line_count} interesting lines\n" if opts[:verbose]

# Join adjacent lines together except where it starts with a <h1>, <h2> etc., <ol>, <li ..>
working_lines = []
...
while n < input_line_count
  ...
  if line.lstrip =~ /(<h1>|<h2>|<h3>|<h4>|<h5>|<ol>|<li )/
    w += 1
...
working_line_count = w
puts "Now reduced to #{working_line_count} working lines\n" if opts[:verbose]
...
version = ''
footnotes = []
number_footnotes = 0 # note counting from 0
n = 0 # note counting from 1
...
# And now process each footnote, simplifying
